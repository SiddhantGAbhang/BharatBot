...
def generate_launch_description():
    ...
    pkg_bharatbot_navigation = get_package_share_directory('bharatbot_navigation')

    gazebo_models_path, ignore_last_dir = os.path.split(pkg_bharatbot_navigation)
    ...
    urdf_file_path = PathJoinSubstitution([
        pkg_bharatbot_navigation,
        "urdf",
        LaunchConfiguration('model')
    ])

    gz_bridge_params_path = os.path.join(
        get_package_share_directory('bharatbot_navigation'),
        'config',
        'gz_bridge.yaml'
    )
    ...
    world_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_bharatbot_navigation, 'launch', 'world.launch.py'),
        ),
        launch_arguments={
    ...
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        arguments=['-d', PathJoinSubstitution([pkg_bharatbot_navigation, 'rviz', LaunchConfiguration('rviz_config')])],
    ...
    ekf_node = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(pkg_bharatbot_navigation, 'config', 'ekf.yaml'),
            {'use_sim_time': LaunchConfiguration('use_sim_time')},
        ]
    )
